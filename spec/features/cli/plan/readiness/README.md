...
Malformed declarations are authored-data errors owned by lint rule `P-009`; readiness MUST conservatively report them unready rather than treating a parser-recovered partial list as ready. This includes duplicate `**Prerequisite Plans:**` headers: the first declaration remains visible to diagnostics, but readiness MUST not traverse any declared edge or report ready. Invalid values MUST be rejected before any plan-file resolution, so a malformed slug cannot escape `spec/plans/`.

For a valid graph, readiness MUST report every direct unmet prerequisite in declaration order followed by its reachable unmet descendants. The sole exception is a diagnostic made entirely of prerequisite-cycle findings, where the cycle itself is the direct actionable refusal.
