...
- On a LEGAL transition, the verb MUST report every file that WOULD change — the target artifact, plus whatever `spec lint --fix` would additionally touch (an ancestor index row, a parent's contents table, or a bidirectional link on a second artifact for the kinds that write one) — with paths relative to the project root, and MUST exit `0`.
