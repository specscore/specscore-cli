...
| `Draft` | `Rejected` | Status rewrite + ideas-index sync |
...
`Draft → Rejected` completes the disposition vocabulary: it is the last pre-terminal status to gain an active-turn-down exit, alongside the passive-decay exit (`Draft → Stale`) it already had. `Draft` is the state where an idea is most likely to be killed deliberately — before this row, that deliberate kill had to be recorded as `Stale`, which the standard defines as passive decay, the wrong word for a decision. `Feature`'s `Draft` already has an active-abandonment exit, spelled `Deprecated` because Feature's disposition set includes it; `Draft → Rejected` is Idea's equivalent of that same door, spelled with Idea's own disposition vocabulary (Idea has no `Deprecated`). With this row, every pre-terminal Idea status can record either an active turn-down or passive decay — the vocabulary is closed.

`Rejected` and `Stale` are not interchangeable at any of these stages: `Rejected` is an active decision against the Idea, `Stale` is passive decay that nobody decided against (see [idea#req:terminal-disposition-transitions](https://github.com/specscore/specscore/blob/main/spec/features/idea/README.md#req-terminal-disposition-transitions)). An Idea that is deliberately cancelled — whether while still in `Draft`, before specification starts, mid-specification, or mid-build — is `Rejected`, not `Stale`.
...
### AC: draft-to-rejected-happy-path

**Requirements:** [cli/idea/change-status#req:legal-transition-matrix](#req-legal-transition-matrix)

Given `spec/ideas/foo.md` containing `**Status:** Draft`, running `specscore idea change-status foo --to=rejected` exits `0`, writes `foo: Draft → Rejected\n` to stdout, and rewrites the Status line to `Rejected` in place. `Draft` is the state where an idea is most likely to be killed deliberately; this Idea can now record that as an active decision rather than being forced into `Stale`, completing the disposition vocabulary across every pre-terminal status.
...
Given `spec/ideas/foo.md` containing `**Status:** Draft`, running `specscore idea change-status foo --to=implementing` exits `4` with a stderr message naming `Draft` as the current status and `Approved`, `In Review`, `Rejected`, `Stale` as the legal targets from `Draft`. No file change.
