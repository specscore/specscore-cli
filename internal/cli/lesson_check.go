...
func runLessonCheck(cmd *cobra.Command, _ []string) error {
	...
	for _, name := range []string{"project", "status", "not-enforced", "min-recurred", "repo", "format"} {
	...
	count, err := countCheckedLessons(lessons, status, notEnforced, min, repo)
	if err != nil {
		return err
	}
	list.SetArgs(listArgsForCheck(project, status, notEnforced, min, repo, format))
...
func countCheckedLessons(items []*lesson.Lesson, status string, notEnforced bool, min int, repo string) (int, error) {
	...
	for _, l := range items {
		...
		if repo != "" && !lessonHasRepository(l, repo) {
			continue
		}
		n := l.Recurred
		if l.Canonical {
...
func listArgsForCheck(project, status string, notEnforced bool, min int, repo, format string) []string {
	args := []string{}
	if project != "" {
